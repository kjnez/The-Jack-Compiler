...
(defun jack-tokenizer (jack-file stream)
  ...
  (let ((lines (get-instructions jack-file)))
	...
	(loop for c across line
		...
		do (when (> j (1+ i)) (print (subseq line i (1- j))) (write-token (subseq line i (1- j)) stream))
		   (setf i j)
		   (when (not (char= #\space c)) (print c) (write-token c stream))
	      when (and (< i (1- j)) (string= #\" c) (string= #\" (char line i)))
		do (format t "~%i: ~d, j: ~d" i j)
		   (print (subseq line i j))
		   (write-token (subseq line i j) stream)
		   (setf i j)
	      ))))
  (format stream "</tokens>~%"))
